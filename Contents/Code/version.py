VERSION = '2024.06.18'

"""
2024.06.18
  - SjvaShow 스캐너 수정   
    에피형식 and 시즌 번호 없음 and 하위폴더 숫자있는 경우 => 무조건 시즌 처리하는 방식에서 Force 형식일 때만 시즌 처리함.   
    단순 파일 분류를 위한 년도별로 파일을 나누었을 때 각 년도별 시즌처리 되는 것을 막음.   

2024.06.11
  - KTV 왓차피디아 추가처리

2024.06.02   
  - MAKEMATE1.E02.240522.1080p-SW.mp4  1회로 인식하는 SjvaShow.py문제 수정
  
2022.08.27
  - yaml music 트랙 정보 추가

2022.08.07
  - add service m3u8

2022.05.27
  - 트랙 rating에 멜론 좋아요 수 매칭.

2022.05.26
  - yaml show에서 시간값 ID가 겹치는 문제 수정

2022.05.14
  - ktv 단일 미디어, 멸티 시즌일때 항상 시즌1로 처리하는 문제 수정

2022.05.12
  - SjvaShow 스캐너 추가

2022.05.10
  - 뮤직관련
  
2022.02.06
  - 쇼 부가영상 update 순서 변경. 파일 부가영상이 온라인 부가영상 보다 먼저 나옴.

2021.12.29-2
 - JAVALL 관련 수정

2021.12.29
 - JAV Uncensored 검색 추가
 - JAV 전체 검색 추가. JAVALL|검색어

2021.12.28
 - JAV search_keyword 오른쪽 - 붙어지는 경우 대응

2021.12.12
 - KTV에서 FT로 tmdb 지정
   예) 닥터 슬럼프 리메이크 FT105262
   
2021.11.28
 - info.json 삭제 오류 수정
 
2021.11.05
 - KTV 시즌처리시 배우가 마지막 시즌만 나오는 문제 수정

2021.11.04
 - FTV에서 KTV검색어로 검색 기능. KTV|검색어
   시리즈 매칭을 위해서 필요

2021.11.03
 - ftv 스튜디오 []으로 나오는 문제 수정

2021.10.21
 -ftv 스튜디어 버그 수정
 
2021.10.21
 - yaml load 수정. (feat. 시우지우아빠)

2021.10.20
 - get_folderpath 수정

2021.10.18
 - "쇼에서 부가영상을 포함할 섹션 ID 목록" 옵션 추가
   F 애니메이션에서 메타새로고침시 부가영상 로딩하지 않기 위해..

2021.10.13
 - 컬렉션 관련 옵션 추가
 
2021.10.12
 - yaml show season 0
 
2021.10.05
 - ftv : 시즌 0도 검색되게
 - lyric URL 생성시에는 key만 넣고, 실제 정보는 메타에서 가져오도록

2021.10.04
 - yaml music 추가
 - yaml dummy agent 추가 : yaml 이외는 다른 처리를 하고 싶지 않을 때

2021.10.01
 - yaml code startswith 체크
 
2021.09.29
 - movie year bug fix
 
2021.09.26
 - JAV DVD로 통합 사용. (검색 실패시 AMA, FC2 순으로 시도)

2021.09.23
 - 기존 jav는 art같은 것이 없는 경우 []를 받았으나 f2c가 추가되면서 null로 리턴하고 있었음.
   필드마다 조건문 추가

2021.09.22
 - 쇼 yaml 파일 처리 추가
 - bug fix

2021.09.15 
 - yaml 라이브러리 포함
 - 영화 yaml 파일 처리 추가

2021.09.07
- jav  xxx-123cd1.json 생성되는 문제 수정

2021.09.02
 - ftv info.json 사용
 
"""
